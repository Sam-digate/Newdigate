...
(function(){
  document.querySelectorAll('[data-demo-preview]').forEach(function(preview){
    ...
    var panels=[].slice.call(preview.querySelectorAll('[data-demo-preview-panel]'));
    if(!tabs.length || !panels.length){return;}

    function activate(tab,moveFocus){
      var target=tab.getAttribute('data-demo-preview-tab');
      ...
      panels.forEach(function(panel){
        var active=panel.getAttribute('data-demo-preview-panel')===target;
        panel.hidden=!active;
...
/* Company belief, system, and principles tabs. */
document.querySelectorAll('[data-company-thinking-tabs]').forEach(function(explorer){
  var tabs=[].slice.call(explorer.querySelectorAll('[role="tab"]'));
  var panels=[].slice.call(explorer.querySelectorAll('[role="tabpanel"]'));
...
(function(){
  document.querySelectorAll('[data-cd-engine-explorer]').forEach(function(explorer){
    ...
    tabs.forEach(function(tab,index){
      tab.addEventListener('mouseenter',function(){activate(index,false);});
...
/* C-Digate offer accordion. */
(function(){
  document.querySelectorAll('[data-cd-pillar-list]').forEach(function(list){
    var buttons=[].slice.call(list.querySelectorAll('.cd-pillar-toggle'));
    if(!buttons.length){return;}

    function openItem(activeButton){
      buttons.forEach(function(button){
        var active=button===activeButton;
        button.setAttribute('aria-expanded',active?'true':'false');
        var item=button.closest('.cd-pillar-item');
        if(item){item.classList.toggle('is-open',active);}
      });
    }

    buttons.forEach(function(button,index){
      button.addEventListener('click',function(){openItem(button);});
...
(function(){
  var carousel=document.querySelector('#pg-l-platform [data-platform-intelligence-carousel]');
  if(!carousel){return;}
  var tabs=[].slice.call(carousel.querySelectorAll('[role="tab"]'));
  var slides=[].slice.call(carousel.querySelectorAll('[role="tabpanel"]'));
  if(tabs.length!==5 || slides.length!==5){return;}
  ...
  function render(index,moveFocus){
    ...
    var previous=(activeIndex-1+slides.length)%slides.length;
    var next=(activeIndex+1)%slides.length;
    var previousFar=(activeIndex-2+slides.length)%slides.length;
    var nextFar=(activeIndex+2)%slides.length;

    slides.forEach(function(slide,slideIndex){
      slide.classList.toggle('is-active',slideIndex===activeIndex);
      slide.classList.toggle('is-prev',slideIndex===previous);
      slide.classList.toggle('is-next',slideIndex===next);
      slide.classList.toggle('is-prev-far',slideIndex===previousFar);
      slide.classList.toggle('is-next-far',slideIndex===nextFar);
  ...
  function start(){
    stop();
    if(reduceMotion || pausedByFocus || document.hidden){return;}
    timer=window.setInterval(function(){render(activeIndex+1,false);},3000);
  }

  tabs.forEach(function(tab,index){
    tab.addEventListener('click',function(){render(index,false);start();});
    tab.addEventListener('keydown',function(event){
      var nextIndex=null;
  ...
  carousel.addEventListener('focusin',function(){pausedByFocus=true;stop();});
